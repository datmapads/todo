...
// Ke thua toan bo chuc nang cua Phien ban 2.1 (dang nhap, CRUD cong viec, muc tieu), bo sung:
//   - Tien do % cho tung cong viec (progress 0-100), co the cap nhat truc tiep tren dashboard
//   - Han chot (due_date) hien thi so ngay con lai / da qua han ngay tren the cong viec
// =========================================================================
...
void LoadDbConfig()
{
    ...
    DB_NAME = dotenv::get("DB_NAME", "todo_db_v22");
...
json GetTasks(int userId)
{
    ...
    std::string q = "SELECT id, name, category, priority, completed, progress, due_date FROM tasks WHERE user_id = " +
...
bool ToggleTask(int userId, int id)
{
    ...
    std::string q;
    if (wasCompleted)
        q = "UPDATE tasks SET completed = 0 WHERE id = " + std::to_string(id) + " AND user_id = " + std::to_string(userId);
    else
        q = "UPDATE tasks SET completed = 1, progress = 100 WHERE id = " + std::to_string(id) + " AND user_id = " + std::to_string(userId);
...
int main()
{
    ...
    std::cout << "== PHIEN BAN 2.2 - DASHBOARD TIEN DO & HAN CHOT ==\n";
